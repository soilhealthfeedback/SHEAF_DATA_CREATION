...
SHEAF_model_data_creation <- function(crop) {
  ...
  crop_damage_loss <- subset(damage_loss, Commodity == crop)
  crop_damage_lossperacre <- subset(damage_lossperacre, Commodity == crop)
  crop_damage_lossperclaim <- subset(damage_lossperclaim, Commodity == crop)
  ...
  library(tidyverse)
  merge1 <- merge(crop_damage_loss,agcensus, by = c("State", "County", "Year"))
